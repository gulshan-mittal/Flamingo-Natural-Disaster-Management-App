...
from decimal import *
from ManagementApp.models import TestTweets
from ManagementApp.tweetutil import *
from ManagementApp.tagger import *
...
def osmap(request):
    return render(request, 'ManagementApp/osmap.html')
...
def index(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect('/ManagementApp/analysis/'+keyword)
    else:
        form = KeywordForm()
...
def analysis_keyword(request, keyword):
    try:
        ...
        template = loader.get_template('ManagementApp/alys_result.html')
        ...
        template = loader.get_template('ManagementApp/error.html')
        context = RequestContext(request, {
            'e_str': str(e),
            })
        return HttpResponse(template.render(context))
...
def tag(request, testtweet_id):
    ...
    template = loader.get_template('ManagementApp/tag.html')
    context = RequestContext(request, {
        'bigrams': bigrams,
        'testtweet': testtweet,
...
def test(request):
    ...
    template = loader.get_template('ManagementApp/test.html')
    context = RequestContext(request, {
        'ratio': str(ratio)
        })
    return HttpResponse(template.render(context))
